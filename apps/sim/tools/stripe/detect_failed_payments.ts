  ...
  directExecution: async (params) => {
    try {
      ...
      charges.forEach((charge) => {
        if (charge.status === 'failed') {
          ...
          if (amount >= minimumAmount) {
            ...
            const customerId = (charge.customer as string) || 'guest'
      ...
      if (categorizedFailures.expired_card > 0) {
        recommendations.push('Send automated emails requesting card updates for expired cards')
      }
        ...
        output: {
          ...
          failure_reasons: Object.entries(failureReasons)
            ...
            .map(([code, count]) => ({
              failure_code: code,
              count,
              percentage: failedPayments.length > 0 ? (count / failedPayments.length) * 100 : 0,
            })),
